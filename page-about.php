...
<div class="about-page">
    ...
    <div class="years-block">
        ...
        <div style="position: relative">
            ...
            <input type="range" min="1" max="1000" value="1" class="scroll-bar_x"></input>
            <div class="list items" id="list" style="overflow-x: scroll">
                ...
                <!-- Убрать -->
                <div class="item"><span>2026</span></div>
                <!-- <div class="item"><span>2026</span></div>
                <div class="item"><span>2026</span></div>
                <div class="item"><span>2026</span></div> -->
...
<script>
    ...
    const x = document.querySelectorAll('.items .item').length
    const koeff = 1 - document.documentElement.clientWidth/slider.scrollWidth
    let isDown = false;
    let startX;
    let walk; 
    let scrollLeft;
    let value;
    const scrollBarSlider = document.querySelector('.scroll-bar_x')
    const scrollBarWidthVW = +getComputedStyle(scrollBarSlider).width.slice(0, -2) 
    const scrollBarLeftVW = +getComputedStyle(scrollBarSlider).left.slice(0, -2)
    console.log(slider.scrollWidth)
    let playFlag = true // проверяет проигрывается ли анимация в данный момент, чтобы не дёргалась
    document.querySelectorAll('.items .item').forEach((item, i, arrItems) => {
        console.log(getComputedStyle(item).width, "   " ,getComputedStyle(item).marginRight)
        slider.scrollWidth += +(getComputedStyle(item).width).slice(0, -2) + +getComputedStyle(item).marginRight.slice(0, -2)
    })
    "mousedown touchstart".split(" ").forEach(element => {
        slider.addEventListener(element, (e) => {
        isDown = true;
        slider.classList.add('active');
        startX = e.pageX - slider.offsetLeft; // координаты мышки относительно левой стороны страницы
        
        scrollLeft = slider.scrollLeft;
        console.log(element, startX, e.pageX, slider.offsetLeft)
    ...
    "mouseup touchend".split(" ").forEach(element => {
        slider.addEventListener(element, () => {
        startinAutoPlay()
        isDown = false;
        slider.classList.remove('active');
    });
    ...
    slider.addEventListener('mousemove', (e) => {
        if(!isDown) return;
        e.preventDefault();
        const x = e.pageX - slider.offsetLeft; // координаты мышки относительно слайдера
        walk = (x - startX) * 1; //scroll-fast
        slider.scrollLeft = scrollLeft - walk;
        scrollBarSlider.value = slider.scrollLeft * 1000 / koeff / slider.scrollWidth
    });
    slider.addEventListener('touchmove', (e)=>{
        scrollBarSlider.value = slider.scrollLeft * 1000 / koeff / slider.scrollWidth
        const IntertInterval = setInterval(()=>{
            scrollBarSlider.value = slider.scrollLeft * 1000 / koeff / slider.scrollWidth
            console.log('hello')
        }, 5)
        setTimeout(()=>{
            clearInterval(IntertInterval)
        }, 600)
    })

    function autoPlay(){
        let scrollToRight = true // направление, куда крутимся
        scrollLeft = slider.scrollLeft;
        window.playInterval = window.setInterval(()=>{
            maxRight = (slider.scrollWidth  - document.documentElement.clientWidth)-scrollLeft // указывает прокрутку до конца: (реальная ширина slider  - ширина экрана) - прокрутка
            if (maxRight >= 0 && scrollToRight){
                scrollLeft += 1
            } else if (maxRight < 0 || !scrollToRight) {
                scrollToRight = false 
                scrollLeft -= 1
            }
            if(scrollLeft <= 0){
    ...
    function startinAutoPlay(){
        if(!playFlag){
            window.startTimeoutPlay = window.setTimeout(()=>{
            autoPlay()
            }, 3000)
        }
        playFlag = true
    ...
    function clearAutoPlay(){
        try{
            ...
            window.clearTimeout(startTimeoutPlay)
        } catch (error){
            console.log('пока не чистим интервал')
        }
        playFlag = false
    }
    autoPlay()// первоначальный запуск

    /////////////////////////////////////////////////// работаем с ползунком
    "click touch".split(" ").forEach(element =>{
        scrollBarSlider.addEventListener(element, ()=>{
            scrollBarSliderListener()
            startinAutoPlay()
        })
    })
    "mousedown touchstart".split(" ").forEach(element => {
        scrollBarSlider.addEventListener(element, ()=>{
            console.log('hello');
            scrollBarSliderListener()
            "mousemove touchmove".split(" ").forEach(element2 => {
            scrollBarSlider.addEventListener(element2, scrollBarSliderListener)
            })
            clearAutoPlay()
        })
    });
    "mouseup touchend".split(" ").forEach(element3 => {
        scrollBarSlider.addEventListener(element3, ()=>{
            console.log('bye')
            scrollBarSliderListener()
            "mousemove touchmove".split(" ").forEach(element4 => {
                scrollBarSlider.removeEventListener(element4, scrollBarSliderListener)
            })
            startinAutoPlay()
        })
    })
    
    function scrollBarSliderListener(){ 
        value = scrollBarSlider.value
        slider.scrollLeft = value / 1000 * koeff * slider.scrollWidth 
        clearAutoPlay()
    }
    ///////////////////////////////////////////////////
    ...
    function selItemScroll() { // подсветка 
        $(".items .item").each(function (index, value) {
            let coordThumb = scrollBarSlider.value / 1000 * scrollBarWidthVW + scrollBarLeftVW // положение ползунка относительно лева документа в px
            let wid_x = $(value).width();
            let pos_x = $(value).position().left;
            if(pos_x <= coordThumb && pos_x+wid_x > coordThumb) {
